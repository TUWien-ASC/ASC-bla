...
template <typename T>   // T is a template parameter, in the compiler it is
                        // replaced by a type (int, float char...)
class Vector {
    /*
  ...
  public:
    Vector(size_t size)                     // Constructor
        : size_(size), data_(new T[size])   // here the ":" is used to
                                            // initialize the member variables
    {
        ;
    }

    Vector(const Vector &v)   // Copy constructor, the const means that the
                              // object v is not changed and the & means that
                              // the object is passed by reference
        : Vector(v.Size())    // here the ":" is used to initialize the member
                              // variables
    {
        *this = v;   // *this is a pointer to
                     // the object itself, so
                     // *this = v means that the
                     // object is assigned to
                     // the object v is it
                     // possible to initializw
                     // the member using "this =
                     // &v" ? No because "this"
                     // is a pointer to the
    ...
    Vector(Vector &&v)               // Move constructor
        : size_{0}, data_(nullptr)   // here the ":" is used to initialize the
                                     // member variables, the curly brackets are
                                     // used to initialize the member variables
                                     // nullptr is a pointer to the address null
    {
...
template <typename T>
std::ostream &
operator<<(std::ostream &ost, const Vector<T> &v) {
    if (v.Size() > 0)
        ost << v(0);
